...
BLOCKER_EXPLANATIONS: Mapping[str, str] = {
    ...
    "NO_CONFIRMATION": "The rejection formed, but the next candle did not close beyond it.",
    "EMA_TREND_NOT_ALIGNED": "The fast EMA is on the wrong side of the slow EMA for this direction, so the setup is filtered out.",
...
ADAPTERS: dict[str, VisualAdapter] = {
    ...
    "three_candle_rejection": VisualAdapter(
        ...
        features=(Feature.SUPPORT, Feature.RESISTANCE, Feature.SWING,
                  Feature.REJECTION_CANDLE, Feature.EMA, Feature.ATR_BAND),
        setup_gates=(
            _g("levels_ready", Stage.FEATURES, "Swing levels, EMA 9/33 and ATR formed",
               ("WARMUP", "WARMING_UP")),
            _g("at_level", Stage.SETUP, "Candle 1 pushed into a level touched 2+ times",
               ("NO_ELIGIBLE_ZONE",)),
            _g("rejection", Stage.SETUP, "Candle 2 wicked through the level and closed back",
               ("NO_SUPPORT_REJECTION", "NO_RESISTANCE_REJECTION")),
            _g("confirmation", Stage.CONFIRMATION, "Candle 3 closed beyond the rejection candle",
               ("NO_CONFIRMATION",)),
            _g("ema_trend", Stage.CONFIRMATION, "EMA 9 on the trade's side of EMA 33",
               ("EMA_TREND_NOT_ALIGNED",)),
        ),
        entry_trigger="Push, rejection and confirmation candles at a support or resistance level.",
        invalidation="Beyond the rejection wick.",
        stop_model="Lower of the rejection and confirmation candles' extremes, plus 0.1 ATR.",
        target_model="2R from entry.",
    ),
