...
export default function PriceActionVisual() {
  ...
  // The entry timeframes the backend offers arrive with each chart. A market
  // switch clears the chart while the new one loads, so keep the last list:
  // otherwise the bar shrank to a lone, unhighlighted "5m" mid-switch.
  const [entryTimeframes, setEntryTimeframes] = useState<string[]>(["5m"]);
  useEffect(() => {
    const offered = state?.mtf_policy?.available_entry_timeframes;
    if (offered?.length) setEntryTimeframes((previous) => previous.join() === offered.join() ? previous : offered);
  }, [state?.mtf_policy?.available_entry_timeframes]);
